...
nanarray = find(isnan(pos));  % find and store any NaN values in the data
...
% Next we scale the right/up/cw-ward calibration pts.	Our starting
...
while i<=numcalpts+1
   ...
   while strcmpi(yorn,'n')
      ...
      temp=input( '--> ' , 's');
      if strcmpi(temp,'q'), return; end
      if strcmpi(temp,'s'), rSkipFlag=1; end
      if strcmpi(temp,'n')
