...
for i = 1:rawlen
   ...
   rest = raw{i}(tabs(1): tabs(end)-1);
   datatxt{i} = [rest, blanks(50-length(rest))];   
end
toc
...
for z = 1:length(block)
   data = str2num(block{z}); %#ok<ST2NM> % because str2double gives 'NaN' for array
   toc
